...
def discover_channel_names() -> list[str]:
    ...
    import openbot.channels as pkg
...
def load_channel_class(module_name: str) -> type[BaseChannel]:
    ...
    from openbot.channels.base import BaseChannel as _Base

    mod = importlib.import_module(f"openbot.channels.{module_name}")
    for attr in dir(mod):
        obj = getattr(mod, attr)
        if isinstance(obj, type) and issubclass(obj, _Base) and obj is not _Base:
            return obj
    raise ImportError(f"No BaseChannel subclass in openbot.channels.{module_name}")
...
def discover_plugins(enabled_names: set[str] | None = None) -> dict[str, type[BaseChannel]]:
    ...
    for ep in entry_points(group="openbot.channels"):
