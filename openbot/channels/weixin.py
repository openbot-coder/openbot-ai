...
from openbot.config.paths import get_media_dir, get_runtime_subdir
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.openbot/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                self.logger.error("login failed. Run 'openbot channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"openbot-{uuid.uuid4().hex[:12]}"
