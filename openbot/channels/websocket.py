"""WebSocket server channel: openbot acts as a WebSocket server and serves connected clients."""
...
from openbot.config.paths import get_media_dir
from openbot.config.schema import Base
from openbot.security.workspace_access import (
    WORKSPACE_SCOPE_METADATA_KEY,
    WorkspaceScopeError,
)
from openbot.session.goal_state import goal_state_ws_blob
from openbot.session.webui_turns import websocket_turn_wall_started_at
from openbot.utils.media_decode import (
    FileSizeExceeded,
    save_base64_data_url,
)
from openbot.webui.cli_apps_api import normalize_cli_app_mentions
from openbot.webui.forking import handle_webui_fork_chat
from openbot.webui.gateway_services import GatewayServices
...
from openbot.webui.mcp_presets_api import normalize_mcp_preset_mentions
from openbot.webui.transcription_ws import webui_transcription_event
from openbot.webui.websocket_logging import websockets_server_logger
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      openbot and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
      blocking ``urllib`` or synchronous ``httpx`` from inside a coroutine.
    - ``token_issue_secret``: If non-empty, token requests must send ``Authorization: Bearer <secret>`` or
      ``X-openbot-Auth: <secret>``.
...
class WebSocketChannel(BaseChannel):
    ...
    async def start(self) -> None:
        from openbot.utils.logging_bridge import redirect_lib_logging
