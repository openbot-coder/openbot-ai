...
helpers from :mod:`openbot.providers.openai_responses`.
...
2. **Microsoft Entra ID (AAD)** — when ``api_key`` is empty the provider
   ...
   is an optional dependency installed via ``pip install openbot-ai[azure]``.
"""
    ...
    def __init__(self, scope: str = _AZURE_OPENAI_SCOPE) -> None:
        try:
            ...
            raise RuntimeError(
                ...
                "Install it with: pip install 'openbot-ai[azure]'"
            ) from exc
