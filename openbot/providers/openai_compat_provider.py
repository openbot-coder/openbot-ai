...
from openbot.providers.base import (
    LLMProvider,
    LLMResponse,
    ToolCallRequest,
...
from openbot.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
def _openai_compat_timeout_s() -> float:
    ...
    return _float_env("openbot_OPENAI_COMPAT_TIMEOUT_S", _OPENAI_COMPAT_REQUEST_TIMEOUT_S)
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply openbot attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _build_client(self) -> None:
        ...
        # Always create an httpx.AsyncClient with trust_env=False so that
        # system proxy settings (e.g. Windows Clash / HTTP_PROXY) do not
        # interfere with TLS handshakes to third-party API servers.
        http_client = httpx.AsyncClient(
            trust_env=False,
            limits=httpx.Limits(keepalive_expiry=0) if self._is_local else httpx.Limits(),
            timeout=timeout_s,
        )
    ...
    async def chat_stream(
        ...
        await self._ensure_client()
        idle_timeout_s = int(os.environ.get("openbot_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(
