...
from openbot.agent import context as agent_context
from openbot.agent import model_presets as preset_helpers
from openbot.agent.autocompact import AutoCompact
from openbot.agent.context import ContextBuilder
from openbot.agent.hook import AgentHook, CompositeHook
from openbot.agent.memory import Consolidator
from openbot.agent.progress_hook import AgentProgressHook
...
from openbot.agent.tools.context import RequestContext, bind_request_context, reset_request_context
from openbot.agent.tools.file_state import FileStateStore, bind_file_states, reset_file_states
...
from openbot.bus.progress import build_bus_progress_callback
from openbot.bus.queue import MessageBus
from openbot.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
    ensure_runtime_event_publisher,
)
from openbot.command import CommandContext, CommandRouter, register_builtin_commands
from openbot.config.schema import AgentDefaults, ModelPresetConfig
...
from openbot.session import turn_continuation
from openbot.session.goal_state import (
    goal_state_runtime_lines,
    runner_wall_llm_timeout_s,
    sustained_goal_active,
)
from openbot.session.manager import Session, SessionManager
from openbot.utils.document import extract_documents, reference_non_image_attachments
from openbot.utils.helpers import image_placeholder_text
from openbot.utils.helpers import truncate_text as truncate_text_fn
...
from openbot.utils.runtime import (
    EMPTY_FINAL_RESPONSE_MESSAGE,
    SUSTAINED_GOAL_CONTINUE_PROMPT,
)

if TYPE_CHECKING:
    from openbot.config.schema import (
        ChannelsConfig,
        ProviderConfig,
        ToolsConfig,
    )
...
class AgentLoop:
    ...
    def __init__(
        ...
        from openbot.config.schema import ToolsConfig
        ...
        # openbot_MAX_CONCURRENT_REQUESTS: <=0 means unlimited; default 3.
        _max = int(os.environ.get("openbot_MAX_CONCURRENT_REQUESTS", "3"))
    ...
    def _set_tool_context(
        ...
        from openbot.agent.tools.context import ContextAware
        ...
        *on_stream_end(resuming)*: called when a streaming session finishes.
        ...
        try:
            result = await self.runner.run(AgentRunSpec(
                ...
                # Sustained goals may legitimately exceed openbot_LLM_TIMEOUT_S; idle stall
                # is still capped by openbot_STREAM_IDLE_TIMEOUT_S in streaming providers.
    ...
    async def run(self) -> None:
        ...
        while self._running:
            try:
                ...
                _is_cancelling = _task.cancelling() if hasattr(_task, "cancelling") else _task.cancelled()
                if not self._running or _is_cancelling:
                    raise
                continue
