...
from openbot.agent.tools.context import current_request_session_key
from openbot.agent.tools.exec_session import (
...
from openbot.agent.tools.sandbox import wrap_command
...
from openbot.security.workspace_access import current_scope_allows_loopback, current_tool_workspace
...
class ExecTool(Tool):
    ...
    def __init__(
        ...
        self.deny_patterns = (deny_patterns or []) + [
            ...
            r":\(\)\s*\{.*\};\s*:",          # fork bomb
            # Block writes to openbot internal state files (#2989).
    ...
    def _wrap_path_export(self, command: str, env: dict[str, str]) -> str:
        segments = []
        if self.path_prepend:
            env["openbot_PATH_PREPEND"] = self.path_prepend
            segments.append("$openbot_PATH_PREPEND")
        segments.append("$PATH")
        if self.path_append:
            env["openbot_PATH_APPEND"] = self.path_append
            segments.append("$openbot_PATH_APPEND")
    ...
    def _guard_command(
        ...
        *,
        ...
        from openbot.security.network import contains_internal_url
