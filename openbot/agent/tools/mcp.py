"""MCP client: connects to MCP servers and wraps their tools as native openbot tools."""
...
from openbot.bus.events import (
...
class MCPToolWrapper(_MCPWrapperBase):
    """Wraps a single MCP server tool as a openbot Tool."""
...
class MCPResourceWrapper(_MCPWrapperBase):
    """Wraps an MCP resource URI as a read-only openbot Tool."""
...
class MCPPromptWrapper(_MCPWrapperBase):
    """Wraps an MCP prompt as a read-only openbot Tool."""
    ...
    for item in structured[:8]:
        ...
        if configured_server_names is not None and raw_name not in configured_server_names:
            lines.append(
                ...
                "tell the user to restart openbot."
            )
            continue
        if connected_server_names is not None and raw_name not in connected_server_names:
            lines.append(
                ...
                "run the preset test, and restart openbot only if hot reload is unavailable."
            )
            continue
        lines.append(
            "MCP Preset Attachment: "
...
async def reload_servers(state: Any, registry: ToolRegistry) -> dict[str, Any]:
    ...
    async with _reload_lock(state):
        try:
            ...
            return {
                "ok": False,
                "message": "Could not reload MCP config. Restart openbot to pick up changes.",
        ...
        if failed:
            ...
            message = "MCP connections refreshed without restarting openbot."
        else:
            message = "MCP config reloaded without restarting openbot."
...
async def request_mcp_reload(bus: Any, *, timeout: float = 15.0) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": False,
            "message": "MCP hot reload timed out. Restart openbot to pick up changes.",
            "requires_restart": True,
        }
...
async def handle_runtime_control(state: Any, msg: InboundMessage, registry: ToolRegistry) -> bool:
    ...
    try:
        ...
        result = {
            "ok": False,
            "message": "MCP hot reload failed. Restart openbot to pick up changes.",
            "requires_restart": True,
            "error": str(exc),
        }
