"""Interactive onboarding questionnaire for openbot."""
...
from openbot.cli.models import (
    format_token_count,
...
def _show_main_menu_header() -> None:
    ...
    from openbot import __logo__, __version__
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]openbot[{__version__}][/bold cyan]")
    )
    console.print()
...
def _handle_fallback_models_field(
    ...
    from openbot.config.schema import InlineFallbackConfig
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from openbot.config.schema to get
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"openbot.channels.{name}")
