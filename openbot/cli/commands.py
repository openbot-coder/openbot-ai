...
# Remove default handler and re-add with unified openbot format
logger.remove()
...
from openbot import __logo__, __version__  # noqa: E402
from openbot.agent.loop import AgentLoop  # noqa: E402
from openbot.cli.stream import StreamRenderer, ThinkingSpinner  # noqa: E402
from openbot.config.paths import get_workspace_path, is_default_workspace  # noqa: E402
from openbot.config.schema import Config  # noqa: E402
from openbot.utils.evaluator import evaluate_response  # noqa: E402
from openbot.utils.helpers import sync_workspace_templates  # noqa: E402
from openbot.utils.restart import (  # noqa: E402
...
app = typer.Typer(
    ...
    help=f"{__logo__} openbot - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from openbot.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} openbot[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} openbot[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} openbot v{__version__}")
        raise typer.Exit()
...
def onboard(
    ...
    """Initialize openbot configuration and workspace."""
    from openbot.config.loader import get_config_path, load_config, save_config, set_config_path
    ...
    if wizard:
        from openbot.cli.onboard import run_onboard
    ...
    agent_cmd = 'openbot agent -m "Hello!"'
    gateway_cmd = "openbot gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} openbot is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/openbot-coder/openbot-ai#-chat-apps[/dim]"
    )
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from openbot.channels.registry import discover_all
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from openbot.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from openbot.config.loader import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from openbot.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show openbot runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'openbot-ai[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger

    from openbot.api.server import create_app
    from openbot.bus.queue import MessageBus
    from openbot.providers.image_generation import image_gen_provider_configs
    from openbot.session.manager import SessionManager

    if verbose:
        logger.enable("openbot")
    else:
        logger.disable("openbot")
...
def _load_or_create_desktop_config(config: str | None, workspace: str | None) -> Config:
    ...
    from openbot.config.loader import (
        get_config_path,
        load_config,
    ...
    from openbot.config.schema import Config as openbotConfig
...
def desktop_gateway(
    ...
    """Start the private local gateway used by openbot Desktop."""
...
def _run_gateway(
    ...
    *,
    ...
    from openbot.channels.manager import ChannelManager
    from openbot.cron.service import CronService
    from openbot.cron.types import CronJob
    from openbot.providers.factory import build_provider_snapshot, load_provider_snapshot
    from openbot.providers.image_generation import image_gen_provider_configs
    from openbot.session.manager import SessionManager
    from openbot.session.webui_turns import WebuiTurnCoordinator
    from openbot.webui.token_usage import TokenUsageHook

    port = port if port is not None else config.gateway.port

    console.print(f"{__logo__} Starting openbot gateway version {__version__} on port {port}...")
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.name == "dream":
            ...
            try:
                ...
                from openbot.webui.token_usage import record_response_token_usage
    ...
    from openbot.cron.types import CronJob, CronPayload, CronSchedule
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show openbot runtime logs during chat"),
):
...
def channels_status(
    ...
    """Show channel status."""
    from openbot.channels.registry import discover_all
    from openbot.config.loader import load_config, set_config_path
...
def plugins_list():
    ...
    from openbot.channels.registry import discover_all, discover_channel_names
    from openbot.config.loader import load_config
...
def status():
    """Show openbot status."""
    from openbot.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} openbot Status\n")
...
def _login_github_copilot() -> None:
    try:
        from openbot.providers.github_copilot_provider import login_github_copilot
