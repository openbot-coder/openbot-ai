...
from openbot.apps.protocol import app_manifest, compact_dict
from openbot.config.paths import get_runtime_subdir
from openbot.security.workspace_policy import is_path_within

CLI_ANYTHING_REGISTRY_URL = "https://hkuds.github.io/CLI-Anything/registry.json"
CLI_ANYTHING_PUBLIC_REGISTRY_URL = "https://hkuds.github.io/CLI-Anything/public_registry.json"
CLI_ANYTHING_RAW_BASE = "https://raw.githubusercontent.com/openbot-coder/CLI-Anything/main"
openbot_EXTENSION_REGISTRY_URL = "https://raw.githubusercontent.com/Re-bin/openbot-extension/main/registry.json"
openbot_EXTENSION_RAW_BASE = "https://raw.githubusercontent.com/Re-bin/openbot-extension/main"
_CATALOG_SOURCES = (
    ...
    ("extensions", openbot_EXTENSION_REGISTRY_URL, openbot_EXTENSION_RAW_BASE, False),
)
...
_BRAND_DOMAINS: dict[str, tuple[str, str]] = {
    ...
    "macrocli": ("github.com/openbot-coder/CLI-Anything/tree/main/macrocli", "#24292F"),
...
class CliAppManager:
    ...
    def _manifest_source(self, app: dict[str, Any]) -> str:
        ...
        if source == "extensions":
            return "openbot-extension"
        return f"cli-anything:{source}"

    def _trust_registry(self, app: dict[str, Any]) -> str:
        return "openbot-extension" if str(app.get("_source") or "") == "extensions" else "cli-anything"
        ...
        *,
        ...
        remove = compact_dict({
            ...
            "verification": (
                ...
                else ["openbot_state_absent", "managed_paths_absent"]
            ),
        })
        return app_manifest(
            app_id=name,
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = _catalog_description(app) or f"Use {display} from openbot."
        return f"""---
...
Use this skill when the user asks openbot to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_openbot_skill_note(self, content: str, app: dict[str, Any]) -> str:
        marker = "<!-- openbot-cli-app-note -->"
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_openbot_skill_note(content, app)
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from openbot. {entry_point} "
                "is still available because it is managed outside openbot."
            )
