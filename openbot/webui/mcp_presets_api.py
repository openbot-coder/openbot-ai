...
from openbot.config.loader import load_config, resolve_config_env_vars, save_config
...
def _custom_manifest(name: str, cfg: MCPServerConfig) -> dict[str, Any]:
    ...
    return app_manifest(
        app_id=name,
        display_name=name,
        description="Custom MCP server from openbot config.",
        category="custom",
        source="mcp-custom",
        brand_color="#64748B",
        capabilities=[
...
async def mcp_presets_test_action(query: QueryParams) -> dict[str, Any]:
    ...
    from openbot.agent.tools.mcp import connect_mcp_servers
