---
name: update-setup
description: One-time setup wizard for the openbot upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `openbot --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v openbot || true
python -m pip show openbot-ai || true
pipx list | sed -n '/openbot-ai/,+3p' || true
uv tool list | sed -n '/openbot-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how openbot was
installed.
...
`question: "Where is your openbot source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "openbot-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "openbot-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "openbot-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade openbot to the latest version. Triggers: upgrade openbot, update openbot, 升级openbot, 更新openbot."
---
...
4. Use `exec` to verify: `openbot --version`
5. Tell the user the new version. Say: "Run `/restart` to restart openbot and apply the update. If `/restart` is unavailable in this channel, restart the openbot process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade openbot' when you want to update."
