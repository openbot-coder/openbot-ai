...
/**
 * Minimal fake WebSocket implementing the subset openbotClient touches.
...
afterEach(() => {
  Reflect.deleteProperty(window, "openbotHost");
  vi.useRealTimers();
});

describe("openbotClient", () => {
  ...
  it("can swap the socket factory when the runtime URL changes", () => {
    ...
    client.close();
    client.updateUrl("openbot-host://engine/", hostFactory);
    client.connect();

    expect(hostFactory).toHaveBeenCalledWith("openbot-host://engine/");
    expect(lastSocket().url).toBe("host:openbot-host://engine/");
  });
  ...
  it("uses the host socket bridge for native host URLs", async () => {
    ...
    Object.defineProperty(window, "openbotHost", {
      configurable: true,
      value: {
        openSocket,
    ...
    expect(openSocket).toHaveBeenCalledWith("openbot-host://engine/");
