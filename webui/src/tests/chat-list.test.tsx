  ...
  it("groups WebUI chats by workspace project while preserving in-project sorting and activity", () => {
    const sessions = [
      ...
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/openbot-bench",
          project_name: "openbot-bench",
          access_mode: "full",
        },
      }),
    ];
    ...
    const openbotSection = screen.getByRole("region", { name: "openbot" });
    const openbotText = openbotSection.textContent ?? "";

    expect(screen.getByRole("region", { name: "openbot-bench" })).toBeInTheDocument();
    expect(within(openbotSection).getByText("Alpha task")).toBeInTheDocument();
    expect(within(openbotSection).getByText("Zeta task")).toBeInTheDocument();
    expect(openbotText.indexOf("Alpha task")).toBeLessThan(openbotText.indexOf("Zeta task"));
    expect(within(openbotSection).getByLabelText("Agent running")).toBeInTheDocument();
  ...
  it("keeps default workspace chats in the Chats section instead of a project folder", () => {
    const sessions = [
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/.openbot/workspace",
    ...
    render(
      <ChatList
        ...
        onRequestRename={vi.fn()}
        onToggleArchive={vi.fn()}
        defaultWorkspacePath="/Users/me/.openbot/workspace"
        showTimestamps
      />,
    );

    expect(screen.getByText("Projects")).toBeInTheDocument();
    expect(screen.getByRole("region", { name: "openbot" })).toBeInTheDocument();
  ...
  it("can collapse a project group and keeps project rename separate from chat titles", async () => {
    ...
    render(
      <ChatList
        ...
        projectNameOverrides={{ "/Users/me/openbot": "Photos" }}
        collapsedGroups={{ "project:/Users/me/openbot": true }}
      />,
    );
    ...
    expect(onToggleGroup).toHaveBeenCalledWith("project:/Users/me/openbot");
    ...
    expect(onNewChatInProject).toHaveBeenCalledWith("/Users/me/openbot", "Photos");
    ...
    expect(onRequestRenameProject).toHaveBeenCalledWith("/Users/me/openbot", "Photos");
  });
