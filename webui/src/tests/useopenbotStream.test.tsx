...
import { useopenbotStream } from "@/hooks/useopenbotStream";
...
describe("useopenbotStream", () => {
  it("batches answer deltas into one animation-frame update", async () => {
    const fake = fakeClient();
    const requestFrame = vi.spyOn(window, "requestAnimationFrame");
    const { result } = renderHook(() => useopenbotStream("chat-batch", EMPTY_MESSAGES), {
  ...
  it("flushes pending delta text before turn_end finalizes the turn", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-flush", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("preserves proactive automation source metadata on complete assistant messages", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-cron", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("drops pending stream work when switching chats", async () => {
    ...
    const { result, rerender } = renderHook(
      ({ chatId }: { chatId: string }) => useopenbotStream(chatId, EMPTY_MESSAGES),
      {
        wrapper: wrap(fake.client),
  ...
  it("starts in streaming mode when history shows pending tool calls", () => {
    ...
    const { result } = renderHook(
      () => useopenbotStream("chat-p", initialMessages, true),
      {
        wrapper: wrap(fake.client),
      },
    );
  ...
  it("collapses consecutive tool_hint frames into one trace row", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-t", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("treats progress with arbitrary agent_ui like ordinary trace text", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-au", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
    act(() => {
      fake.emit("chat-au", {
  ...
  it("renders live tool traces from structured tool events", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-tool-events", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("dedupes finish-phase tool events after their start trace", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-tool-finish", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps phase updates when a tool event trace line is deduped", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-tool-phase", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("renders live file_edit events as their own activity trace", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-file-edit", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("replaces matching write_file tool events with live file edit activity", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-file-edit-events", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("upgrades pending file_edit placeholders when the path arrives", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-file-edit-pending", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("merges file_edit updates after interleaved progress events", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-file-edit-progress", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps interrupted pre-tool text as assistant output before activity", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-stream-segments", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("does not replace interrupted pre-tool text with final stream_end text", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-stream-end-final", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("splits live assistant output around tool hints without moving it into reasoning", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-live-segments", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("opens a new activity segment for reasoning after file edit activity", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-file-segments", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps file edit blocks ordered across a new reasoning phase", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-file-order", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("accumulates reasoning_delta chunks on a placeholder until reasoning_end", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-r", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("absorbs a streaming reasoning placeholder into the answer turn that follows", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-r2", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("ignores empty reasoning_delta frames", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-r3", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("treats legacy kind=reasoning messages as a complete delta + end pair", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-r4", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("starts a new Thought block when reasoning arrives after visible output", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-r5", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("stamps completed live Thought blocks with their own latency", async () => {
    ...
    try {
      const fake = fakeClient();
      const { result } = renderHook(() => useopenbotStream("chat-r5-lat", EMPTY_MESSAGES), {
        wrapper: wrap(fake.client),
      });
      await act(async () => {});
  ...
  it("keeps alternating reasoning and answer deltas in separate ordered blocks", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-r5b", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("does not attach a new turn's reasoning across the latest user boundary", async () => {
    ...
    const { result } = renderHook(
      () => useopenbotStream("chat-r6", initialMessages),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("does not attach reasoning across a tool trace boundary", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-r7", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps tool-call reasoning before the matching live tool trace", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-tool-reasoning", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("absorbs non-streamed final answers into the preceding reasoning placeholder", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-final-reasoning", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("prunes reasoning-only placeholders when a turn ends without an answer", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-empty-thinking", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("drops stale reasoning-only placeholders before sending the next user turn", () => {
    ...
    const { result } = renderHook(
      () => useopenbotStream("chat-stale-thinking", initialMessages),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("attaches assistant media_urls to complete messages", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-m", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps assistant html media as a file attachment", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-html-media", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("infers assistant svg media as an image attachment", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-svg-media", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("corrects explicit image media when the name is a non-image file", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-mislabelled-html", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("suppresses redundant stream confirmation after assistant media", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-img-result", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("passes image generation options to the websocket client", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-img", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("stops the active turn without adding a user slash command bubble", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-stop", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps streaming alive across stream_end and completes on turn_end", async () => {
    ...
    const { result } = renderHook(() => useopenbotStream("chat-s", EMPTY_MESSAGES, false, onTurnEnd), {
      wrapper: wrap(fake.client),
    });
  ...
  it("replaces streamed content with final stream_end text when provided", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-stream-final", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("creates an assistant bubble from final stream_end text without prior delta", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-stream-end-only", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("stamps latency on the last assistant bubble from turn_end", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-lat", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("tracks goal_status running and clears on idle", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useopenbotStream("chat-g", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
