...
const WS_OPEN = 1;
const WS_CLOSING = 2;
const HOST_SOCKET_URL_PREFIX = "openbot-host://";
 ...
 * - **Production**: off unless ``localStorage.setItem('openbot_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('openbot_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("openbot_debug_ws")?.trim().toLowerCase() ?? "";
...
export class openbotClient {
  ...
  constructor(private options: openbotClientOptions) {
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    try {
      ...
      if (wsInboundDebugEnabled()) {
        ...
        console.warn(
          "[openbot ws inbound] invalid JSON",
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[openbot ws inbound]", summarizeInboundWsPayload(parsed));
    }
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - openbotClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }
