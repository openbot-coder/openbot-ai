...
This page gets one local openbot reply working. After that, you can add the WebUI, chat apps, local models, web search, MCP, deployment, or custom plugins.
...
If your shell cannot find `openbot` after a pip install, run the module form:

```bash
python -m openbot --version
python -m openbot onboard
```

On Windows, `~` in the docs means your user profile directory, for example `C:\Users\you`.

The docs use `python` in commands. If your system exposes Python 3.11+ as `python3` or `py`, use that command in the same place, for example `python3 -m pip install openbot-ai` or `py -m openbot --version`.
...
| Path | What it is |
|------|------------|
| `~/.openbot/config.json` | Main settings file for providers, models, channels, tools, gateway, and API |
| `~/.openbot/workspace/` | Agent workspace for memory, sessions, heartbeat tasks, skills, and artifacts |

If you already have a config, `openbot onboard` can refresh missing default fields without overwriting your existing values.

## 3. Configure a Provider

Skip this section if you already configured provider and model settings in the wizard.

Open `~/.openbot/config.json`. Add or merge these blocks into the file created by `openbot onboard`; do not replace the whole file unless you want to reset the config.
...
`apiBase` is the HTTP base URL of the provider endpoint, not the model name. Most hosted providers in openbot already know their default endpoint, so you usually only set `apiKey` and a model preset. Set `apiBase` when you are using:
...
If you prefer not to store secrets in `config.json`, reference an environment variable and set it before starting openbot:
...
- the `openbot` command is installed;
- `~/.openbot/config.json` can be loaded;
...
After the interactive session can answer normally, openbot can help with its own next setup step. Ask it to read the relevant docs, inspect your current `~/.openbot/config.json`, and make one concrete change such as enabling WebUI, adding a provider preset, or configuring one chat channel. When openbot says the config is updated, run `/restart` in the chat or restart the openbot process manually so long-running processes reload `config.json`.

Example prompt:

```text
Read docs/quick-start.md, docs/providers.md, and docs/configuration.md in this checkout.
Then update ~/.openbot/config.json to add an OpenRouter model preset named "primary".
...
```bash
python -m pip install -U openbot-ai
openbot --version
```
...
|---------|---------------|
| `openbot: command not found` | Use `python -m openbot ...`, or add your Python scripts directory to `PATH`. |
| `ModuleNotFoundError: openbot` | Confirm you installed into the same Python environment that is running the command. |
| JSON parse errors | Check commas and braces in `~/.openbot/config.json`; examples above are partial snippets to merge. |
...
| The CLI works but a chat app does not reply | First keep `openbot gateway` running, then follow [`chat-apps.md`](./chat-apps.md). |
