...
from openbot.bus.events import OUTBOUND_META_AGENT_UI, OutboundMessage
from openbot.bus.queue import MessageBus
from openbot.channels.websocket import (
    WebSocketChannel,
    WebSocketConfig,
    _is_valid_chat_id,
    _parse_envelope,
...
from openbot.config.loader import load_config, save_config
from openbot.config.schema import Config, ModelPresetConfig
from openbot.session import webui_turns as wth
from openbot.session.manager import SessionManager
from openbot.webui.gateway_services import GatewayServices, build_gateway_services
from openbot.webui.http_utils import (
...
from openbot.webui.settings_api import settings_payload, update_provider_settings
from openbot.webui.transcript import append_transcript_object, read_transcript_lines
...
def isolate_webui_workspace_state(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr("openbot.config.paths.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "openbot.webui.workspaces.get_webui_dir",
...
def test_issue_route_secret_matches_bearer_and_header() -> None:
    ...
    x_headers = Headers([("X-openbot-Auth", "my-secret")])
...
async def test_webui_message_envelope_marks_inbound_metadata(bus: MagicMock) -> None:
    from openbot.webui.transcript import read_transcript_lines
...
async def test_webui_message_envelope_persists_user_transcript_for_refresh(
    ...
    from openbot.webui.transcript import build_webui_thread_response, read_transcript_lines
...
async def test_webui_user_transcript_append_failure_does_not_block_inbound(
    ...
    monkeypatch.setattr("openbot.webui.transcript.append_transcript_object", fail_append)
    channel = _ch(bus)
    conn = AsyncMock()
    conn.remote_address = ("127.0.0.1", 50123)
...
async def test_send_stages_external_media_as_signed_url(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("openbot.channels.websocket.get_media_dir", fake_media_dir)
    monkeypatch.setattr("openbot.webui.media_gateway.get_media_dir", fake_media_dir)
    channel = WebSocketChannel({"enabled": True, "allowFrom": ["*"]}, bus, gateway=_basic_handler(bus))
    mock_ws = AsyncMock()
    channel._attach(mock_ws, "chat-1")
...
def test_registry_discovers_websocket_channel() -> None:
    from openbot.channels.registry import load_channel_class
...
async def test_settings_api_returns_safe_subset_and_updates_whitelist(
    ...
    monkeypatch.setattr(
        "openbot.webui.settings_api._oauth_provider_status",
        lambda _spec: {
            "configured": False,
            "account": None,
            "expires_at": None,
    ...
    try:
        ...
        assert workspace_path.endswith("/.openbot/workspace")
...
async def test_bootstrap_exposes_native_surface(bus: MagicMock) -> None:
    ...
    try:
        response = await _http_get(
            ...
            headers={"X-openbot-Auth": "native-secret"},
        )
        assert response.status_code == 200
        body = response.json()
...
def test_settings_payload_reports_workspace_sandbox(monkeypatch, tmp_path) -> None:
    ...
    save_config(config, config_path)
    monkeypatch.setattr("openbot.config.loader._current_config_path", config_path)
    monkeypatch.setenv("openbot_SANDBOX_ENFORCED", "macos_app_sandbox")
...
def test_sessions_list_includes_active_run_started_at(monkeypatch) -> None:
    ...
    from openbot.session import webui_turns as wth
    from openbot.webui import ws_http as ws_http_module
...
def test_handle_webui_thread_get_returns_json(tmp_path, monkeypatch) -> None:
    ...
    from openbot.webui.transcript import append_transcript_object
...
def test_handle_file_preview_returns_workspace_file(tmp_path) -> None:
    ...
    workspace = tmp_path / "workspace"
    source = workspace / "openbot" / "agent" / "hook.py"
    ...
    enc = quote(key, safe="")
    path = quote("openbot/agent/hook.py:12", safe="")
    req = Request(
    ...
    assert resp.status_code == 200
    body = json.loads(resp.body.decode())
    assert body["display_path"] == "openbot/agent/hook.py"
...
def test_file_preview_normalizes_windows_file_url() -> None:
    from openbot.webui.file_preview import _clean_preview_path
