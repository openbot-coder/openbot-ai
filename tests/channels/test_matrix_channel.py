...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("openbot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "openbot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("openbot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "openbot.channels.matrix.asyncio.create_task", _fake_create_task
    )
...
async def test_fetch_media_rejects_missing_declared_size(monkeypatch, tmp_path) -> None:
    ...
    channel.client = client
    monkeypatch.setattr("openbot.channels.matrix.get_media_dir", lambda _name: tmp_path)
