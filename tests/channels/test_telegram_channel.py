...
class _FakeBot:
    ...
    async def get_me(self):
        self.get_me_calls += 1
        return SimpleNamespace(id=999, username="openbot_test")
...
async def test_start_creates_separate_pools_with_proxy(monkeypatch) -> None:
    ...
    monkeypatch.setattr("openbot.channels.telegram.HTTPXRequest", _FakeHTTPXRequest)
    monkeypatch.setattr(
        "openbot.channels.telegram.Application",
...
async def test_send_delta_stream_end_html_expansion_does_not_overflow() -> None:
    ...
    """
    from openbot.channels.telegram import _markdown_to_telegram_html
...
async def test_send_delta_incremental_edit_splits_oversized_buffer() -> None:
    ...
    from openbot.channels.telegram import TELEGRAM_MAX_MESSAGE_LEN
...
async def test_send_remote_media_url_after_security_validation(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr("openbot.channels.telegram.validate_url_target", lambda url: (True, ""))
...
async def test_send_blocks_unsafe_remote_media_url(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr(
        "openbot.channels.telegram.validate_url_target",
...
async def test_group_policy_mention_accepts_text_mention_and_caches_bot_identity() -> None:
    ...
    await channel._on_message(_make_telegram_update(text="@openbot_test hi", entities=[mention]), None)
    await channel._on_message(_make_telegram_update(text="@openbot_test again", entities=[mention]), None)
...
async def test_group_policy_mention_accepts_caption_mention() -> None:
    ...
    await channel._on_message(
        _make_telegram_update(caption="@openbot_test photo", caption_entities=[mention]),
        None,
    )

    assert len(handled) == 1
    assert handled[0]["content"] == "@openbot_test photo"
...
async def test_download_message_media_returns_path_when_download_succeeds(
    ...
    monkeypatch.setattr(
        "openbot.channels.telegram.get_media_dir",
...
async def test_forward_command_pairs_unauthorized_private_user(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr(
        "openbot.channels.base.generate_code", lambda _ch, _sid: "ABCD-EFGH"
    )
...
async def test_forward_command_preserves_dream_log_args_and_strips_bot_suffix() -> None:
    ...
    channel._handle_message = capture_handle
    update = _make_telegram_update(text="/dream-log@openbot_test deadbeef", reply_to_message=None)
...
async def test_forward_command_normalizes_telegram_safe_dream_aliases() -> None:
    ...
    channel._handle_message = capture_handle
    update = _make_telegram_update(text="/dream_restore@openbot_test deadbeef", reply_to_message=None)
...
def test_telegram_bus_slash_command_regex_matches_agent_loop_commands() -> None:
    ...
    assert pat.fullmatch("/skill@openbot_bot")
    assert pat.fullmatch("/new@openbot_bot")
    assert pat.fullmatch("/goal@openbot_bot refine objective")
...
def test_strip_md_block_removes_inline_formatting() -> None:
    from openbot.channels.telegram import _strip_md_block
