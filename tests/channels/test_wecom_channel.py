...
from openbot.channels.wecom import (
    WecomChannel,
    WecomConfig,
...
async def test_download_and_save_success() -> None:
    ...
    with patch("openbot.channels.wecom.get_media_dir", return_value=Path(tempfile.gettempdir())):
...
async def test_process_image_message() -> None:
    ...
    try:
        with patch("openbot.channels.wecom.get_media_dir", return_value=Path(os.path.dirname(saved))):
            frame = _FakeFrame(body={
                "msgid": "msg_img_1",
                "chatid": "chat1",
                "from": {"userid": "user1"},
...
async def test_process_file_message_uses_sdk_filename_when_name_missing(tmp_path: Path) -> None:
    ...
    with patch("openbot.channels.wecom.get_media_dir", return_value=tmp_path):
        frame = _FakeFrame(body={
