...
async def test_openai_compat_stream_forwards_reasoning_deltas_deepseek_style() -> None:
    ...
    with patch("openbot.providers.openai_compat_provider.AsyncOpenAI") as mock_openai:
...
async def test_openrouter_sets_default_attribution_headers() -> None:
    spec = find_by_name("openrouter")
    with patch("openbot.providers.openai_compat_provider.AsyncOpenAI") as mock_client_cls:
        provider = OpenAICompatProvider(
    ...
    assert headers["HTTP-Referer"] == "https://github.com/openbot-coder/openbot-ai"
    assert headers["X-OpenRouter-Title"] == "openbot"
...
async def test_openrouter_user_headers_override_default_attribution() -> None:
    ...
    with patch("openbot.providers.openai_compat_provider.AsyncOpenAI") as mock_client_cls:
        provider = OpenAICompatProvider(
            ...
            default_model="anthropic/claude-sonnet-4-5",
            extra_headers={
                "HTTP-Referer": "https://openbot.ai",
                "X-OpenRouter-Title": "openbot Pro",
    ...
    headers = mock_client_cls.call_args.kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://openbot.ai"
    assert headers["X-OpenRouter-Title"] == "openbot Pro"
    assert headers["X-OpenRouter-Categories"] == "cli-agent,personal-agent"
...
async def test_openrouter_keeps_model_name_intact() -> None:
    ...
    with patch("openbot.providers.openai_compat_provider.AsyncOpenAI") as MockClient:
...
async def test_openai_compat_stream_watchdog_returns_error_on_stall(monkeypatch) -> None:
    monkeypatch.setenv("openbot_STREAM_IDLE_TIMEOUT_S", "0")
