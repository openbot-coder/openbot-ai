...
async def test_runner_uses_raw_messages_when_context_governance_fails():
    from openbot.agent.runner import AgentRunSpec, AgentRunner
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("openbot.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
    ...
    monkeypatch.setattr(
        "openbot.agent.runner.estimate_message_tokens",
...
def test_snip_history_reserves_budget_for_tool_definitions(monkeypatch):
    ...
    monkeypatch.setattr("openbot.agent.runner.estimate_prompt_tokens_chain", _estimate)
    token_sizes = {
        "system": 50,
        "old user": 200,
        "old assistant": 200,
...
async def test_backfill_missing_tool_results_inserts_error():
    ...
    from openbot.agent.runner import AgentRunner, _BACKFILL_CONTENT
...
def test_drop_orphan_tool_results_removes_unmatched_tool_messages():
    from openbot.agent.runner import AgentRunner
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from openbot.agent.loop import AgentLoop
    from openbot.agent.runner import _BACKFILL_CONTENT
...
async def test_runner_backfill_only_mutates_model_context_not_returned_messages():
    ...
    from openbot.agent.runner import AgentRunSpec, AgentRunner, _BACKFILL_CONTENT
...
async def test_microcompact_replaces_old_tool_results():
    ...
    from openbot.agent.runner import AgentRunner, _MICROCOMPACT_KEEP_RECENT
...
def test_snip_history_preserves_user_message_after_truncation(monkeypatch):
    ...
    monkeypatch.setattr("openbot.agent.runner.estimate_prompt_tokens_chain", lambda *_a, **_kw: (500, None))
