...
from openbot.command.builtin import cmd_new, register_builtin_commands
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("openbot.agent.loop.SessionManager"), \
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates openbot onboard)."""
