...
def test_bootstrap_files_are_backed_by_templates() -> None:
    template_dir = pkg_files("openbot") / "templates"
...
def test_identity_has_no_behavioral_instructions(tmp_path) -> None:
    ...
    assert "You are openbot" not in identity
...
def test_default_soul_template_contains_execution_rules() -> None:
    ...
    soul = (pkg_files("openbot") / "templates" / "SOUL.md").read_text(encoding="utf-8")
...
def test_template_memory_md_is_skipped(tmp_path) -> None:
    ...
    assert "This file is automatically updated by openbot" not in prompt
