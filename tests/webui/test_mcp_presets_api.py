...
from openbot.webui.mcp_presets_api import (
    McpPresetError,
    custom_mcp_action,
    mcp_presets_action,
    mcp_presets_payload,
...
def _use_config(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("openbot.config.loader._current_config_path", tmp_path / "config.json")
...
def test_test_mcp_preset_reports_missing_dependency(
    ...
    _use_config(tmp_path, monkeypatch)
    mcp_presets_action("enable", {"name": ["playwright"]})
    monkeypatch.setattr("openbot.webui.mcp_presets_api.shutil.which", lambda _command: None)
...
def test_test_mcp_preset_connects_and_reports_tools(
    ...
    monkeypatch.setattr("openbot.agent.tools.mcp.connect_mcp_servers", fake_connect)
