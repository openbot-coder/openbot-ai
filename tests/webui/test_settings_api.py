...
from openbot.webui.settings_api import (
    WebUISettingsError,
...
def test_settings_payload_includes_oauth_provider_status(
    ...
    monkeypatch.setattr("openbot.webui.settings_api._oauth_provider_status", fake_oauth_status)
...
def test_settings_payload_includes_network_safety_fields(
    ...
    save_config(config, config_path)
    monkeypatch.setattr("openbot.config.loader._current_config_path", config_path)
    monkeypatch.setattr("openbot.webui.workspaces.get_webui_dir", lambda: tmp_path / "webui")
...
def test_settings_payload_includes_token_usage_summary(
    ...
    save_config(config, config_path)
    monkeypatch.setattr("openbot.config.loader._current_config_path", config_path)
    monkeypatch.setattr("openbot.webui.token_usage.get_webui_dir", lambda: tmp_path / "webui")

    from openbot.webui.token_usage import record_token_usage
...
def test_provider_models_payload_fetches_openai_compatible_models(
    ...
    monkeypatch.setattr("openbot.webui.settings_api.httpx.get", fake_get)
...
def test_azure_openai_spec_no_longer_requires_api_key() -> None:
    ...
    from openbot.webui.settings_api import _provider_requires_api_key
