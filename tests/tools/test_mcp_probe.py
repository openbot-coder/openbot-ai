...
from openbot.agent.tools.mcp import _probe_http_url, connect_mcp_servers
...
async def test_connect_skips_unreachable_streamable_http():
    ...
    with patch("openbot.agent.tools.mcp._probe_http_url", _unreachable):
...
async def test_probe_not_called_for_stdio():
    ...
    with patch("openbot.agent.tools.mcp._probe_http_url", _spy_probe):
        cfg = MagicMock()
        cfg.type = "stdio"
        cfg.url = None
