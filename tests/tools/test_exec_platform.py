...
class TestBuildEnvUnix:
    ...
    def test_expected_keys(self):
        with patch("openbot.agent.tools.shell._IS_WINDOWS", False):
    ...
    def test_secrets_excluded(self, monkeypatch):
        ...
        monkeypatch.setenv("openbot_TOKEN", "tok-secret")
        with patch("openbot.agent.tools.shell._IS_WINDOWS", False):
            env = ExecTool()._build_env()
        assert "OPENAI_API_KEY" not in env
        assert "openbot_TOKEN" not in env
        for v in env.values():
...
class TestBuildEnvWindows:
    ...
    def test_expected_keys(self):
        with patch("openbot.agent.tools.shell._IS_WINDOWS", True):
            env = ExecTool()._build_env()
    ...
    def test_path_has_sensible_default(self):
        with (
            patch("openbot.agent.tools.shell._IS_WINDOWS", True),
...
class TestSpawnUnix:
    ...
    async def test_uses_bash(self):
        with (
            patch("openbot.agent.tools.shell._IS_WINDOWS", False),
...
class TestPathAppendPlatform:
    ...
    async def test_unix_uses_env_var_in_fixed_export(self):
        ...
        with (
            patch("openbot.agent.tools.shell._IS_WINDOWS", False),
            patch("openbot.agent.tools.shell.os.pathsep", ":"),
        ...
        assert captured_cmd == 'export PATH="$PATH:$openbot_PATH_APPEND"; ls'
        assert captured_env["openbot_PATH_APPEND"] == "/opt/bin; echo INJECTED"
    ...
    async def test_unix_path_prepend_uses_env_var_in_fixed_export(self):
        ...
        assert captured_cmd == 'export PATH="$openbot_PATH_PREPEND:$PATH"; python --version'
        assert captured_env["openbot_PATH_PREPEND"] == "/venv/bin; echo INJECTED"
        assert "INJECTED" not in captured_cmd
    ...
    async def test_unix_path_prepend_and_append_order(self):
        ...
        assert captured_cmd == (
            'export PATH="$openbot_PATH_PREPEND:$PATH:$openbot_PATH_APPEND"; python --version'
        )
        assert captured_env["openbot_PATH_PREPEND"] == "/venv/bin"
        assert captured_env["openbot_PATH_APPEND"] == "/usr/sbin"
...
class TestSandboxPlatform:
    ...
    async def test_bwrap_applied_on_unix(self):
        ...
        with (
            patch("openbot.agent.tools.shell._IS_WINDOWS", False),
            patch("openbot.agent.tools.shell.wrap_command", return_value="bwrap -- sh -c ls") as mock_wrap,
            patch.object(ExecTool, "_spawn", return_value=mock_proc) as mock_spawn,
            patch.object(ExecTool, "_guard_command", return_value=None),
        ):
