...
If you discover a security vulnerability in openbot, please report it by:
...
chmod 600 ~/.openbot/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.openbot/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.openbot/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run openbot with a dedicated user account
...
- Keep authentication data in `~/.openbot/whatsapp-auth` secure (mode 0700)
...
pip install --upgrade openbot-ai
```
...
- Subscribe to security advisories for openbot and its dependencies
...
1. **Isolate the Environment**
   ...
   pip install openbot-ai
   ```

2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash openbot
   sudo -u openbot openbot gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.openbot
   chmod 600 ~/.openbot/config.json
   chmod 700 ~/.openbot/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.openbot/logs/openbot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.openbot` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.openbot/logs/openbot.log
   ```
