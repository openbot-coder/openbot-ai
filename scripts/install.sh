...
install_failure_hint() {
  printf '%s\n' "Error: pip could not install openbot from $install_source." >&2
  ...
  printf '  %s\n' "$python_bin -m openbot onboard --wizard" >&2
  exit 1
}
...
By default this installs or upgrades openbot-ai from PyPI.
...
if [ -n "$python_bin" ]; then
  ...
  "$python_bin" - <<'PY' >/dev/null 2>&1 || fail "openbot requires Python 3.11 or newer"
import sys
raise SystemExit(0 if sys.version_info >= (3, 11) else 1)
PY
else
...
if [ "$dry_run" = "1" ]; then
  info "Dry run: would install or upgrade openbot from $install_source."
  ...
  if [ "${openbot_SKIP_WIZARD:-}" = "1" ]; then
    info "Dry run: would skip setup wizard because openbot_SKIP_WIZARD=1."
  else
    info "Dry run: would run: $python_bin -m openbot onboard --wizard"
  fi
  info "Dry run: no changes made."
  exit 0
fi

info "Installing or upgrading openbot from $install_source..."
...
info "Installed openbot:"
"$python_bin" -m openbot --version

if [ "${openbot_SKIP_WIZARD:-}" = "1" ]; then
  info "Skipping setup wizard because openbot_SKIP_WIZARD=1."
  info "Run this later: $python_bin -m openbot onboard --wizard"
  exit 0
fi

info "Starting setup wizard..."
"$python_bin" -m openbot onboard --wizard

info "Done. Try: $python_bin -m openbot agent -m \"Hello!\""
