...
const APP_PROTOCOL = "openbot-app:";
const APP_HOST = "app";
const HOST_SOCKET_PROTOCOL = "openbot-host:";
...
function repoRoot(): string {
  return process.env.openbot_DESKTOP_REPO_ROOT
    ? path.resolve(process.env.openbot_DESKTOP_REPO_ROOT)
...
function webDistPath(root: string): string {
  if (process.env.openbot_DESKTOP_WEB_DIST) {
    return path.resolve(process.env.openbot_DESKTOP_WEB_DIST);
  }
  const bundled = path.join(process.resourcesPath, "openbot-webui");
  if (app.isPackaged && existsSync(path.join(bundled, "index.html"))) {
    return bundled;
  }
  return path.join(root, "openbot", "web", "dist");
}

function webDevUrl(): string | null {
  const value = process.env.openbot_DESKTOP_WEB_DEV_URL?.trim();
...
function desktopContentSecurityPolicy(devUrl: string | null): string {
  const connectSrc = ["'self'", "openbot-host:"];
  if (devUrl) {
  ...
  return [
    ...
    "img-src 'self' data: blob: https: openbot-app:",
    "font-src 'self' data:",
...
function pythonExecutable(): string {
  if (process.env.openbot_DESKTOP_PYTHON) {
    return path.resolve(process.env.openbot_DESKTOP_PYTHON);
  }
  const bundled = path.join(bundledResourcePath("openbot-engine"), "bin", "python3");
...
function appendGatewayLogs(gateway: ChildProcess, logsDir: string): void {
  ...
  gateway.stdout?.on("data", (chunk) => {
    stream.write(chunk);
    process.stdout.write(`[openbot] ${chunk}`);
  });
  gateway.stderr?.on("data", (chunk) => {
    stream.write(chunk);
    process.stderr.write(`[openbot] ${chunk}`);
  });
  gateway.once("exit", (code, signal) => {
    stream.write(`\n[openbot] engine exited code=${code ?? ""} signal=${signal ?? ""}\n`);
    stream.end();
  });
}
...
function notifyRuntimeStatus(status: EngineStatus): void {
  ...
  sendToRenderer(mainWindow?.webContents, "openbot:runtime-status", status);
}
...
function scheduleCrashRestart(gateway: ChildProcess): void {
  ...
  setTimeout(() => {
    ...
    void startRuntime().catch((error) => {
      console.error("failed to restart openbot engine", error);
      notifyRuntimeStatus("crashed");
    });
  }, 1000);
}
...
async function proxyToGateway(request: Request): Promise<Response> {
  ...
  if (requestUrl.pathname === "/webui/bootstrap") {
    headers.set("X-openbot-Auth", runtime.secret);
  }
  const init: {
    body?: ArrayBuffer;
    headers: Headers;
  ...
  return Response.json({
    ...body,
    ws_url: `openbot-host://engine${normalizedWsPath}`,
...
function registerAppProtocol(webDist: string, devUrl: string | null): void {
  protocol.handle("openbot-app", async (request) => {
...
function registerIpcHandlers(): void {
  handleHostIpc("openbot:get-runtime-info", () => runtimeInfo());
  handleHostIpc("openbot:restart-engine", async () => {
    await restartRuntime();
  });
  handleHostIpc("openbot:pick-folder", async () => {
  ...
  handleHostIpc("openbot:open-logs", async () => {
    const logsDir = runtime?.logsDir ?? userDataPath("logs");
    await mkdir(logsDir, { recursive: true });
    const error = await shell.openPath(logsDir);
    if (error) throw new Error(error);
  });
  handleHostIpc("openbot:export-diagnostics", async () => {
    const diagnosticsPath = path.join(
      app.getPath("temp"),
      `openbot-diagnostics-${Date.now()}.json`,
    );
    await writeFile(
      diagnosticsPath,
  ...
  handleHostIpc("openbot:check-for-updates", () => ({
    supported: false,
    message: "Auto update is not configured for this build.",
  }));
  handleHostIpc("openbot:ws-connect", (event, rawUrl) => {
    if (!runtime) throw new Error("openbot engine is not running");
    const url = parseHostSocketUrl(rawUrl);
    const id = randomBytes(12).toString("hex");
    const client = new UnixWebSocketClient(runtime.socketPath, url, {
      onOpen: () => sendToRenderer(event.sender, "openbot:ws-event", { id, type: "open" }),
      onMessage: (data) => {
        handleDesktopNotificationFrame(data, { getWindow: () => mainWindow });
        sendToRenderer(event.sender, "openbot:ws-event", { id, type: "message", data });
      },
      onError: (message) => sendToRenderer(event.sender, "openbot:ws-event", { id, type: "error", message }),
      onClose: (code, reason) => {
        hostSockets.delete(id);
        sendToRenderer(event.sender, "openbot:ws-event", { id, type: "close", code, reason });
      },
    });
  ...
  handleHostIpc("openbot:ws-send", (_event, id, data) => {
  ...
  handleHostIpc("openbot:ws-close", (_event, id) => {
    if (typeof id !== "string") {
...
async function loadAppWindow(win: BrowserWindow): Promise<void> {
  ...
  await win.loadURL("openbot-app://app/index.html");
}
