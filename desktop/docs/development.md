...
This guide is for GitHub contributors who want to change the desktop app. If you are using openbot rather than developing it, the important bit is simpler: desktop runs the local engine for you and shows the same chat, settings, apps, skills, and workspace UI as the browser WebUI.

`desktop` is the native host for the shared openbot WebUI. It is not a fork of the WebUI, and it should not grow a second copy of product UI.

The healthy mental model is:

```text
openbot core  -> agent runtime, gateway, providers, tools, memory
...
For source checkouts, the host starts the engine with local `python3` and injects the repository root into `PYTHONPATH`. This means Python changes under `openbot/` are picked up from the current checkout.
...
| Change | Location |
| --- | --- |
| Agent behavior, tools, providers, memory, config schema | `openbot/` |
...
For example, if desktop Settings needs an "Open logs" button, the button belongs in the shared WebUI settings page because it is product UI. The actual filesystem operation belongs in the desktop host and is exposed through `window.openbotHost`.

## Host Contract

The shared WebUI talks to desktop through `window.openbotHost`. WebUI code may check for host capabilities, but it must not import Electron, Node.js modules, or desktop source files.
...
4. Does browser WebUI still work when `window.openbotHost` is missing?
5. Does the engine behavior belong in openbot core instead of Electron?
...
- Do not commit `desktop/node_modules`, `desktop/build`, `desktop/dist`, DMGs, or `desktop/resources/openbot-engine`.

## Release Shape

Release builds assemble three existing parts:

1. the shared WebUI build from `openbot/web/dist`,
2. the Python engine prepared under `desktop/resources/openbot-engine`,
